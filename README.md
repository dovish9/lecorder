...
2. 대시보드를 열 때 Whisper 모델은 로드하지 않습니다.
3. 전사 작업 시 large-v3와 Silero VAD를 로드하고, 마지막 음성 인식 후 60초간 새 작업이 없으면 종료해 메모리를 반환합니다. 유예 시간 안에 시작한 작업은 모델을 재사용합니다.
...
4. **Whisper 1차 전사:** 긴 음성을 약 120초 단위로 나눕니다. 가까운 침묵 지점에서 자르고, 침묵이 없을 때는 경계 양쪽에 2초 문맥을 포함합니다. 요청마다 이전 전사의 텍스트 상태를 초기화하고 각 구간 안에서는 문맥과 Silero VAD를 사용합니다. 힌트는 디코더 문맥을 과도하게 차지하지 않도록 UTF-8 400바이트까지 전달합니다.
5. **품질 보호:** 반복이 감지된 구간만 이전 텍스트 문맥과 VAD를 끄고 beam search로 한 번 재시도합니다. 재시도도 반복되면 잘못된 결과를 저장하지 않고 실패를 표시합니다. VAD 누락 의심 구간 역시 해당 구간만 다시 읽습니다.
...
| `WHISPER_URL` | 미설정 | 명시하면 외부 Whisper 서버 사용; 로드·종료는 외부에서 관리 |
| `WHISPER_IDLE_SECONDS` | `60` | 마지막 음성 인식 후 모델 해제까지의 유예 시간 |
...
전사 변경 후에는 자동 테스트에 더해 실제 음성을 실행합니다. 이 명령은 임시 작업 폴더를
사용하므로 원본 음성, 운영 DB와 결과 저장소를 변경하지 않습니다. `--llm`을 추가하면
Qwen 검수까지 수행합니다. 종료 시 테스트가 로드한 Whisper 모델을 해제합니다.

```bash
WHISPER_PORT=18080 python3 scripts/verify_transcription.py \
  "experiments/samples/physics2/record/물리학2-9월16일-수.mp4" \
  --language en --report experiments/verification.json
```
